...
export const QuestionCard = ({ question, onAnswer, onNext }: QuestionCardProps) => {
  ...
  const handleFeedback = (questionId: string, feedback: string, type: "positive" | "negative" | "error") => {
    submitFeedback(questionId, feedback, type);
  };
  ...
  const verificationStatus = getQuestionStatus(question.id);
      ...
      {showResult && (
        <>
          ...
          <QuestionFeedback
            ...
            questionSource={question.sources?.textbook || question.sources?.olympiad}
